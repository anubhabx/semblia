...
- Next implementation checkpoint: continue moving one existing mock-backed `web_v2` surface at a time onto live hooks. API keys and analytics are the next visible candidates, but both need deliberate mock-shaped prop adaptation rather than a blind hook swap.
