...
Last updated: 2026-05-19 (project Settings rebuild)
...
- Next implementation checkpoint: delegate the two Settings-related backend gaps to Codex as separate phases — (1) `GET /v2/projects/:slug/public-surface-hosts` listing endpoint with `V2PublicSurfaceHostDTO`, and (2) email-based invite path for `addProjectMember`. After each lands, swap the corresponding Hosts/Members UI placeholders for the live data. Analytics dashboard wiring remains the next mock-backed surface after that.
