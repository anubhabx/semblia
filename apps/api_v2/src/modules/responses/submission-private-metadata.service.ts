...
type PrivateMetadataWriter = {
  formResponsePrivateMetadata: {
    create(args: {
      data: Prisma.FormResponsePrivateMetadataUncheckedCreateInput;
    }): Promise<unknown>;
  };
};
...
export class SubmissionPrivateMetadataService {
  ...
  async createForPublicSubmit(
    ...
    return writer.formResponsePrivateMetadata.create({
      data: {
  ...
  private getEncryptionKeyOrThrow() {
    ...
    if (!key) {
      throw new InternalServerErrorException(
        "Response private metadata requires API_V2_SECRET_ENCRYPTION_KEY",
      );
    }
